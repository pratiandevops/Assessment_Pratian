var connectionHost = "http://172.30.13.153:2501";
// var userName = window.name;
(function (mod) {
    if (typeof exports == "object" && typeof module == "object") // CommonJS        
        mod(require("../plugin/codemirror/lib/codemirror"));
    else if (typeof define == "function" && define.amd) // AMD
        define(["../plugin/codemirror/lib/codemirror"], mod);
...
function clickFunction() {
    var data = document.getElementById("codemirror").value;
    console.log(data);
    alert(data);
}

var mode = "text/x-csharp";
...
function setLanguage() {
    ...
    if (choice === 'C#') {
        mode = "text/x-csharp"
        document.getElementById('codemirror').value = "c#";
...
{                         
    class Praleso 
    {
        ...
        mode = "text/x-csrc"
        // document.getElementsByTagName('pre')[0].childNodes[0].innerText = "<!--Write your C Program-->";
        document.getElementById('userArgument').value = "";
        document.getElementById('text-area').value = "";
        editor.setOption('mode', mode);
...
{       
    ...
    return 0; 
        ...
        mode = "text/x-python"
        document.getElementById('codemirror').value = "python";
        document.getElementById('userArgument').value = "";
        document.getElementById('text-area').value = "";
        editor.setOption('mode', mode);
...
`);
        ...
        mode = "text/x-java"
        document.getElementById('codemirror').value = "Java";
        document.getElementById('userArgument').value = "";
        document.getElementById('text-area').value = "";
        editor.setOption('mode', mode);
        editor.getDoc().setValue(
            `// Don't change the Class name
public class Praleso
{
    ...
    // else {
    //     mode = "text/html"
    //     document.getElementById('codemirror').value = "html";
    //     editor.setOption('mode', mode);
    // }
}
...
function codeSubmit() {
    editor.save();
    var yes = confirm(`Do you want to save the code \nonce save the code done you can't change or modify `);
    
    if (yes) {     
        // var userName = document.getElementsByClassName("username")[0].innerText;
        // var moduleId = document.getElementsByClassName("course-header")[0].baseURI;
        var code = document.getElementById("codemirror").value;
        var codeSelected = select.value;
        const URL = `${connectionHost}/submitCode`;        
        let fetchData = {
            method: 'POST',
            body: JSON.stringify({
                "code": code,
                "userName": "userName",
                "language": codeSelected,
                "moduleId": "moduleId"
            }),
        ...
        fetch(URL, fetchData)
            ...
            .then((responseJson) => {
                ...
                if (responseJson.output) {
                    data = `${responseJson.output.passedCase} testcase passed and ${responseJson.output.failedCase} testcase failed`;
                }
...
function submit_html() {
    ...
    const curl = `${connectionHost}/ccompiler`;
    const purl = `${connectionHost}/pcompiler`;
    const jurl = `${connectionHost}/jcompiler`;
    const csharp = `${connectionHost}/csharpcompiler`

    let fetchData = {
        method: 'POST',
        body: JSON.stringify({ "code": code, "arg": arg }),
    ...
    if (select.value === "C" || select.value === "C++") {
        fetch(curl, fetchData)
            .then(resonse => resonse.json())
            .then((responseJson) => {
                console.log(responseJson);
                var data = '';
    ...
    // document.getElementById("result").value = responseJson.output;
}
