...
var glotProxy = proxy({
    ...
    headers: {
        'Authorization': 'Token da732786-2b0d-4fd1-b218-d19064119883'
    },
    changeOrigin: true,
    pathRewrite: {
        '^/glotapi/': '/',
    ...
    if (language === "C" || language === "C++") {
        compiler.compileCPPWithTestCase(envData, code, userName, language, moduleId, function(data) {
            ...
            if (data.error) {
                ...
                var passedData = data.filter((data) => {
                    return data.testStatus === "passed"
                });
                var failedData = data.filter((data) => {
                    return data.testStatus === "failed"
                });
                result = {
                    output: {
                        passedCase: passedData.length,
                        failedCase: failedData.length
                    }
                };
            }
            return res.send(result);
